...
import { saveAnonymizedBill, saveBillToPatientSession } from './services/integrationService';
import { 
...
const App = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Header Visibility
      if (currentScrollY > lastScrollY && currentScrollY > 150) setIsHeaderVisible(false);
  ...
  useEffect(() => {
    const handleHash = () => {
        ...
        else if (hash === 'dashboard') setView(AppView.PATIENT_DASHBOARD);
  ...
  const NavLink = ({ label, href, onClick }: { label: string, href?: string, onClick?: () => void }) => (
    <button 
      onClick={onClick || (() => { if(href) window.location.hash = href; })}
      className="group relative px-2 py-1"
    >
      <span className="text-[11px] font-black uppercase tracking-[0.3em] italic text-slate-400 group-hover:text-white transition-colors duration-300">
        {label}
      </span>
      <span className="absolute bottom-0 left-0 w-0 h-[2px] bg-cyan-500 transition-all duration-500 group-hover:w-full" />
    </button>
  );
  ...
  const LandingPage = () => {
    ...
    return (
      <div className="animate-fade-in bg-[#0F172A] text-white overflow-x-hidden selection:bg-cyan-500/30">
          ...
          <div className="max-w-6xl mx-auto text-center relative z-10">
            <div className="flex flex-col items-center space-y-12 animate-slide-up">
              <div className="inline-flex items-center gap-2 px-6 py-2 bg-rose-500/10 border border-rose-500/20 rounded-full">
                ...
                <span className="text-[10px] md:text-xs font-black uppercase tracking-[0.3em] text-rose-500 italic">Global Recovery Node: ACTIVE</span>
              </div>
              ...
              <div className="flex flex-col items-center gap-8 pt-8">
                <Button 
                  variant="teal" 
                  glow 
                  className="h-28 px-24 text-2xl font-black italic uppercase shadow-[0_30px_90px_rgba(6,182,212,0.3)] hover:scale-105 transition-transform bg-[#06B6D4] text-white" 
        ...
        {/* CORE VALUE SECTION - THE FORENSIC STANDARD */}
        <section className="py-40 px-6 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                  ...
                  <Card key={i} className="bg-white p-12 py-16 rounded-[3.5rem] shadow-2xl flex flex-col items-center text-center group hover:-translate-y-2 transition-transform duration-500">
                      <div className="w-16 h-16 bg-[#0F172A] rounded-2xl flex items-center justify-center mb-8">{card.icon}</div>
                      <h3 className="text-2xl font-black text-slate-900 uppercase italic tracking-tighter mb-4">{card.title}</h3>
                      <p className="text-slate-500 text-sm font-medium italic">{card.desc}</p>
                  </Card>
              ))}
            </div>
        </section>

        {/* REVIEWS SECTION - THE WALL OF RECOVERY */}
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row items-end justify-between mb-24 gap-8">
              ...
              <div className="flex gap-12">
                <div className="text-right">
                  ...
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic mt-1">Total Recovered</p>
                </div>
                <div className="text-right">
                  <p className="text-4xl md:text-5xl font-black text-white italic tracking-tighter">84%</p>
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic mt-1">Audit Win-Rate</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                ...
                <Card key={i} className="bg-white p-10 md:p-12 rounded-[3.5rem] border border-white/10 relative flex flex-col justify-between h-auto hover:-translate-y-2 transition-all duration-500 shadow-xl">
                  <div>
                    <div className="flex items-center gap-1 mb-8">
                      {[1,2,3,4,5].map(s => <Star key={s} size={16} className="text-amber-500 fill-amber-500" />)}
                    </div>
                    <p className="text-slate-900 text-xl italic leading-relaxed font-bold mb-12">
                      "{review.story}"
                    </p>
                  </div>
                  <div className="flex items-center justify-between border-t border-slate-100 pt-10">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 bg-slate-900 rounded-[1.25rem] flex items-center justify-center font-black italic text-white text-xl"> {review.name[0]} </div>
                      <div>
                        <p className="font-black text-slate-950 uppercase italic tracking-tighter text-lg leading-tight">{review.name}</p>
                        <p className="text-[10px] text-slate-400 font-black uppercase italic tracking-widest mt-1">{review.location}</p>
                      </div>
                    </div>
        ...
        <section className="py-40 px-6 max-w-4xl mx-auto">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="bg-slate-900 border border-white/5 rounded-3xl overflow-hidden transition-all">
                <button 
          ...
          <div className="max-w-4xl mx-auto relative z-10 space-y-12">
            ...
            <div className="flex flex-col items-center gap-8">
              <Button 
                    variant="teal" 
                    glow
                    className="h-28 px-24 text-2xl font-black italic uppercase shadow-3xl bg-cyan-400 text-slate-950 hover:bg-white hover:text-rose-600 transition-all" 
                    onClick={() => { window.location.hash = 'scan'; }}
                >
  ...
  return (
    <div className="min-h-screen font-sans bg-[#0F172A] relative selection:bg-cyan-500 selection:text-white">
      ...
      <header 
        className={`fixed top-0 left-0 w-full z-[100] px-6 transition-all duration-700 pointer-events-none
          ${isHeaderVisible ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}
          ${isScrolled ? 'pt-6' : 'pt-10'}
        `}
      >
        <nav className={`mx-auto max-w-7xl flex items-center justify-between pointer-events-auto transition-all duration-700
          ${isScrolled 
            ? 'h-20 bg-slate-900/80 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] px-10 shadow-[0_20px_50px_rgba(0,0,0,0.5),0_0_20px_rgba(6,182,212,0.1)]' 
        ...
        `}>
            ...
            <div className="flex flex-col">
              ...
              {!isScrolled && (
                <div className="flex items-center gap-1.5 mt-1">
                  ...
                  <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest italic leading-none">System Integrity Verified</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-12">
            <NavLink label="Vault" href="dashboard" />
            <NavLink label="Practitioners" href="advocates" />
            <NavLink label="Legal Shield" onClick={() => setView(AppView.LANDING)} />
            <div className="h-6 w-[1px] bg-white/10 mx-2" />
            <Button 
              onClick={() => { window.location.hash = 'scan'; }} 
              variant="teal"
              className={`font-black italic uppercase transition-all duration-700
                ${isScrolled 
                  ? 'h-12 px-8 text-[10px] tracking-widest' 
                  : 'h-14 px-10 text-[11px] tracking-[0.2em] shadow-lg shadow-cyan-500/20'}
              `}
            >
              Audit Bill
            </Button>
          ...
          {/* MOBILE TOGGLE */}
          <button 
            className={`lg:hidden p-4 rounded-2xl bg-white/5 border border-white/10 text-white transition-all
      ...
      {/* MOBILE TACTICAL MENU OVERLAY */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[150] bg-slate-950 flex flex-col items-center justify-center p-12 animate-fade-in lg:hidden">
            <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_50%_50%,rgba(6,182,212,0.1)_0%,transparent_70%)] pointer-events-none" />
            
            <div className="relative z-10 flex flex-col items-center gap-10 text-center w-full">
                <BrandLogo size="lg" glow className="mb-8" />
                
                {[
                  { label: "My Secure Vault", href: "dashboard" },
                  { label: "Advocate Portal", href: "advocates" },
                  { label: "Clinical Standards", href: "" },
                  { label: "HIPAA Safety", href: "" }
                ].map((item, i) => (
                  <button 
                    key={i}
                    style={{ animationDelay: `${i * 100}ms` }}
                    className="animate-slide-up text-4xl font-black italic uppercase tracking-tighter text-white hover:text-cyan-400 transition-colors"
                    onClick={() => { window.location.hash = item.href; setIsMobileMenuOpen(false); }}
                  >
                    {item.label}
                  </button>
                ))}
                ...
                <Button 
                  onClick={() => { window.location.hash = 'scan'; setIsMobileMenuOpen(false); }} 
                  variant="teal" 
                  glow
                  className="h-28 w-full text-2xl text-slate-950 font-black uppercase italic"
                >
                ...
                <button 
                  ...
                  className="mt-16 text-slate-500 hover:text-white transition-colors"
                >
                  <X size={48} />
                </button>
            </div>
      ...
      <main className="min-h-screen">
        {view === AppView.LANDING ? <LandingPage /> : 
         ...
         view === AppView.PATIENT_DASHBOARD ? <PatientDashboard onScanNew={() => { window.location.hash = 'scan'; }} onViewBill={() => {}} /> :
      ...
      <footer className="bg-[#0F172A] pt-40 pb-20 border-t border-white/10 px-8 text-center">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12 mb-24">
             <div className="flex flex-col items-center md:items-start gap-4">
                ...
                <p className="text-slate-500 text-sm italic font-medium max-w-xs text-center md:text-left">The clinical-grade operating system for medical bill verification and financial aid matching.</p>
             </div>
             <div className="grid grid-cols-2 md:grid-cols-3 gap-16 text-left">
                ...
                <div className="space-y-4">
                   ...
                   <ul className="space-y-2 text-sm text-slate-500 italic">
                      <li className="hover:text-cyan-400 cursor-pointer">HIPAA Safety</li>
                      <li className="hover:text-cyan-400 cursor-pointer">SOC-2 Verified</li>
                ...
                <div className="space-y-4 hidden md:block">
                   ...
                   <ul className="space-y-2 text-sm text-slate-500 italic">
                      <li className="hover:text-cyan-400 cursor-pointer">Case Tracking</li>
                      <li className="hover:text-cyan-400 cursor-pointer">Expert Help</li>
                      <li className="hover:text-cyan-400 cursor-pointer">API Access</li>
                   </ul>
                </div>
             </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto flex flex-col items-center gap-8 mb-12">
            <div className="flex items-center gap-3 px-6 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
              ...
              <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest italic">System Status: PRODUCTION_ONLINE</span>
            </div>
          </div>

          <LegalDisclaimer />
          <div className="mt-12 text-[11px] font-black uppercase tracking-[0.5em] text-slate-700 italic">© 2025 PocketProof. Medical Fraud OS • AES-256 Verified.</div>
      </footer>
    </div>
  );
};
